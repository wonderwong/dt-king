...
function dt_test_save_session() {
	if( !empty($_SESSION['dt_captcha']) && !isset($GLOBALS['dt_captcha_save']) )
		$GLOBALS['dt_captcha_save'] = $_SESSION['dt_captcha'];
}
dt_test_save_session();
...
//        $this->whoami = $str;
    ...
    function check( $number, $place ) {
		if( !isset($_SESSION['dt_captcha']) && isset($GLOBALS['dt_captcha_save']) )
			$_SESSION['dt_captcha'] = $GLOBALS['dt_captcha_save'];
...
function dt_check_current_date() {
    ...
    if( !wp_verify_nonce($nonce,'dt_contact_'.$place) ) {
        ...
        if( !empty($phone) ) {
            $msg_mail .= "Telephone: ". $phone. "\n";
        }
        
        if( !empty($website) ) {
            $msg_mail .= "Website: ". $website. "\n";
        }
        
        $msg_mail .= "Message: ". $msg. "\n";
        
        $send = wp_mail(
            $em,
            'Feedback from: '. esc_attr(home_url()),
            $msg_mail,
			$headers
        );
