...
/*                var body = ed.getBody();
                if (tinymce.isIE){ 
                  //Disable layer resizing
                    body.oncontrolselect = function(e) {
                        return false; 
                    };   
                };
                if (tinymce.isGecko) ed.settings.object_resizing = false;
                        
                //Workaround for IE where it messes up the JS engine if you insert a layer on IE 6,7
                if (tinymce.isIE) dom.setHTML(body, body.innerHTML);
*/            });
        ...
        _doScImage: function( co ){
            ...
            return co.replace( t.regex, function(a,b,c){
				// add new classes
				var new_class = b.replace('_', '-');
				var column = c.match(/column="(.*?)"/);
				if( column && column.length == 2 )
					new_class = new_class + ' ' + column[1];
				else
					new_class = new_class + ' full-width';
				
                return '<p class="helper-tag"><img src="'+t.shortcodes[b].image+'" id="vscImage'+(t.counter++)+'-'+b+'" class="mceItem jpbVisualShortcode ' + new_class + '" data-src="' + b + tinymce.DOM.encode(c) + '" /></p>';
            });
        },
