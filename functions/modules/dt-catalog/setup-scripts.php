...
function dt_catalog_related_works_script() {
    global $post;
    if( empty($post) || 'dt_catalog' != get_post_type($post->ID) ) {
        return false;
    }
    ?>
    <script type="text/javascript">
        jQuery(document).ready(function() {
            ...
            var dt_rel_block = jQuery('#dt_catalog-post_related');
            ...
            if( jQuery('.dt_admin_show_related:checked', dt_rel_block).length ) {
                ...
                if( jQuery('.dt_admin_other_cat:checked', dt_rel_block).length ) {
...
add_action( 'admin_print_scripts', 'dt_catalog_related_works_script', 99 );
