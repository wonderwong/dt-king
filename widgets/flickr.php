			...
			if ($photos)
			{
			   ...
			   $counter=1;
				foreach($photos["items"] as $key => $value)
				{
					...
					$out .= $before_item . "<a target=\"_blank\" class=\"alignleft-f\" $thickbox_attrib href=\"$href\"><img alt=\"\" title=\"\" src=\"$photo_url\" width=\"58\" height=\"58\" /><i></i></a>$photo_title" . "" .$after_item;
...
function sakura_widget_quickflickr_control() {
    ...
    <p><label for="flickr-items"><?php echo("How many items?"); ?> <input class="widefat" id="rss-items" name="sakura-rss-items" value="<?php echo $items; ?>" /></label></p>
